...
config | `{canvas, webgl2, ownRunLoop, cameraConfig, glContextConfig, allowedDevices, layers}` |Configuration parameters to pass to [`XR8.run()`](/docs/api/engine/xr8/run) as well as PlayCanvas specific configuration, e.g. `layers`.
...
ownRunLoop [Optional] | `Boolean` | `false` | If true, XR should use it's own run loop.  If false, you will provide your own run loop and be responsible for calling [`XR8.runPreRender()`](/docs/api/engine/xr8/runprerender) and [`XR8.runPostRender()`](/docs/api/engine/xr8/runpostrender) yourself [Advanced Users only]
