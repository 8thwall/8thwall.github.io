...
[xr:screenshotrequest](xrscreenshotrequest.md) | Emits a request to the engine to capture a screenshot of the PlayCanvas canvas. The engine will emit a [`xr:screenshotready`](/docs/api/engine/playcanvasevents/xrscreenshotready) event with the JPEG compressed image or [`xr:screenshoterror`](/docs/api/engine/playcanvasevents/xrscreenshoterror) if an error has occured.
