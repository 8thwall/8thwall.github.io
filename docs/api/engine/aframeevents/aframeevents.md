...
------------- | -----------
[camerastatuschange](camerastatuschange.md) | This event is emitted when the status of the camera changes. See [`onCameraStatusChange`](/docs/api/engine/camerapipelinemodule/oncamerastatuschange) from [`XR8.addCameraPipelineModule()`](/docs/api/engine/xr8/addcamerapipelinemodule) for more information on the possible status.
[realityerror](realityerror.md) | This event is emitted when an error has occured when initializing 8th Wall Web. This is the recommended time at which any error messages should be displayed. The [`XR8.XrDevice()` API](/docs/api/engine/xrdevice) can help with determining what type of error messaging should be displayed.
...
[screenshoterror](screenshoterror.md) | This event is emitted in response to the [`screenshotrequest`](/docs/api/engine/aframeeventlisenters/screenshotrequest) event resulting in an error.
[screenshotready](screenshotready.md) | This event is emitted in response to the [`screenshotrequest`](/docs/api/engine/aframeeventlisenters/screenshotrequest) event being being completed successfully. The JPEG compressed image of the AFrame canvas will be provided.
...
[xrtrackingstatus](xrtrackingstatus.md) | This event is emitted when [`XR8.XrController`](/docs/api/engine/xrcontroller) starts and any time tracking status or reason changes.
