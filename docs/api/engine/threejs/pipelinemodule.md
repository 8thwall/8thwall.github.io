...
A pipeline module that interfaces with the three.js environment and lifecyle. The three.js scene can be queried using [`XR8.Threejs.xrScene()`](xrscene.md) after [`XR8.Threejs.pipelineModule()`](pipelinemodule.md)'s [`onStart`](/docs/api/engine/camerapipelinemodule/onstart) method is called. Setup can be done in another pipeline module's [`onStart`](/docs/api/engine/camerapipelinemodule/onstart) method by referring to [`XR8.Threejs.xrScene()`](xrscene.md) as long as [`XR8.addCameraPipelineModule()`](/docs/api/engine/xr8/addcamerapipelinemodule) is called on the second module *after* calling `XR8.addCameraPipelineModule(XR8.Threejs.pipelineModule())`.

* [`onStart`](/docs/api/engine/camerapipelinemodule/onstart), a three.js renderer and scene are created and configured to draw over a camera feed.
* [`onUpdate`](/docs/api/engine/camerapipelinemodule/onupdate), the three.js camera is driven with the phone's motion.
* [`onRender`](/docs/api/engine/camerapipelinemodule/onrender), the renderer's `render()` method is invoked.
...
[`XR8.GlTextureRenderer.pipelineModule()`](/docs/api/engine/gltexturerenderer/pipelinemodule) before installing this
...
A three.js pipeline module that can be added via [`XR8.addCameraPipelineModule()`](/docs/api/engine/xr8/addcamerapipelinemodule).
