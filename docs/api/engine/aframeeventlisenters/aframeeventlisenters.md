...
[screenshotrequest](screenshotrequest.md) | Emits a request to the engine to capture a screenshot of the AFrame canvas. The engine will emit a [`screenshotready`](/docs/api/engine/aframeevents/screenshotready) event with the JPEG compressed image or [`screenshoterror`](/docs/api/engine/aframeevents/screenshoterror) if an error has occured.
