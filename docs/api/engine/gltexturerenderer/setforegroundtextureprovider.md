...
Sets a provider that passes a list of foreground textures to draw. This should be a function that take the same inputs as [`cameraPipelineModule.onUpdate`](/docs/api/engine/camerapipelinemodule/onupdate).
...
processGpuResult | `Object` | Data returned by all installed modules during [`onProcessGpu`](/docs/api/engine/camerapipelinemodule/onprocessgpu).
processCpuResult | `Object` | Data returned by all installed modules during [`onProcessCpu`](/docs/api/engine/camerapipelinemodule/onprocesscpu).
