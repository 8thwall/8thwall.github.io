...
--------- | -----------
config [Optional] | Configuration parameters to pass to [`XR8.run()`](/docs/api/engine/xr8/run)
faceConfig [Optional] | Face configuration parameters to pass to [`XR8.FaceController`](/docs/api/engine/facecontroller)
...
ownRunLoop [Optional] | `Boolean` | `true` | If true, XR should use it's own run loop.  If false, you will provide your own run loop and be responsible for calling [`runPreRender`](/docs/api/engine/xr8/runprerender) and [`runPostRender`](/docs/api/engine/xr8/runpostrender) yourself [Advanced Users only]
